...
func (s SongRepositoryImpl) GetSongById(id string) (res model.Song, err error) {
	...
	err = s.DB.Where("song_id = ?", id).Preload("User").Preload("Play").Preload("Album").Find(&res).Error
	return
}
...
func (s SongRepositoryImpl) GetSongByArtist(artistId string) (res []model.Song, err error) {
	err = s.DB.Where("user_id = ?", artistId).Preload("User").Preload("Play").Preload("Album").Find(&res).Error
	return
}

func (s SongRepositoryImpl) GetSongByAlbum(albumId string) (res []model.Song, err error) {
	err = s.DB.Where("album_id = ?", albumId).Preload("User").Preload("Play").Preload("Album").Find(&res).Error
	return
}
